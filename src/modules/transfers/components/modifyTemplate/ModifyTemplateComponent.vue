  ...
  <div>
    ...
    <div class="breadcrumb-trail m-b-32 m-t-8">
      ...
      <p :class="`vel-text-small vel-text-semibold ${companyName}`">
        {{ $t('TRANSFERS-MODULE.STEP-TEMPLATES.BREADCRUMB.MODIFY') }}
      </p>
    </div>
    <div class="m-b-18">
      <p
    ...
    <div class="container-buttons m-t-16">
      ...
      >
        ...
        <velocity-button
          v-if="!templateCreated && currentSubStep === SUBSTEPS.stepThree"
          type="button"
          size="medium"
          @action-button="addStep()"
          classes-name="btn btn-secundary"
          :text="$t('TRANSFERS-MODULE.STEP-TEMPLATES.BUTTON.SAVE-NEW')"
        >
        </velocity-button>
        <velocity-button
          type="button"
          size="medium"
          @action-button="addStep(true)"
          classes-name="btn btn-primary m-a"
          :text="
            templateCreated
              ? $t('REUSABLES.BUTTON.CLOSE')
              : currentSubStep == SUBSTEPS.stepThree
              ? $t('REUSABLES.BUTTON.SAVE')
...
<style scoped lang="sass" src="./ModifyTemplateComponent.sass"></style>
